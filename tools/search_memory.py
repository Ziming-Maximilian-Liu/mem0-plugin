...
class SearchMemoryTool(Tool):
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage, None, None]:
        ...
        # Map app_id to agent_id for namespace isolation: app_id takes priority if both are provided
        agent_id = tool_parameters.get("agent_id")
        app_id = tool_parameters.get("app_id")
        run_id = tool_parameters.get("run_id")
        # Use app_id as agent_id for isolation if app_id is provided, otherwise use agent_id
        effective_agent_id = app_id if app_id else agent_id
